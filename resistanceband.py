...
class ResistanceBand():
    ...
    def __init__(self, band):
        self.band = band
        self.click_count = 0
        self.band.after(1000, self.open_resband)
        self.band.title("FitInclusive")
        self.band.rowconfigure(0, weight=1)
        self.band.columnconfigure(0, weight=1)
        height = 720
        width = 1280
        x = (self.band.winfo_screenwidth() // 2) - (width // 2)
        y = (self.band.winfo_screenheight() // 2) - (height // 2)
        self.band.geometry('{}x{}+{}+{}'.format(width, height, x, y))
        self.band.resizable(0, 0)
        self.band.config(bg="#291C4F")
        self.myfont = CTkFont(family='Poppins', size=14, weight='bold', slant='italic')

        self.img_path1 = "D:\\pythonProject1\\pict\\Resistance band.png"
        self.img_path2 = "D:\\pythonProject1\\pict\\resband.png" #if not self.is_go else "C:\\Users\\Hanseok\\Documents\\project sda\\bgrounded4.png"
        self.img_path3 = "D:\\pythonProject1\\pict\\backbutton2.png"
        self.load_image()
        self.main_frame()

        def exit():
            sure = messagebox.askyesno("Exit", "Are you sure you want to exit", parent=self.band)
            if sure:
                self.band.destroy()
        
        self.band.protocol("WM_DELETE_WINDOW", exit)
    ...
    def main_frame(self):
        ...
        frame_utama = CTkFrame(self.band, fg_color="#291C4F", bg_color="#291C4F")
    ...
    def tts_resband(self):
        speech = gTTS("Resistance band exercise", lang="en")
        filepath = ("rbe.mp3")
        speech.save(filepath)
        try:
            playsound.playsound(filepath)
...
class Timer:
    def __init__(self, timer):
        ...
        self.calories_list = [7, 7, 7, 7, 7, 7, 7, 7, 7, 7]
        self.total_time = 0
        self.total_calories = 0

        self.exercise_names = ['Resistance Band Bicep Curls', 'Resistance Band Rows', 'Resistance Band Lateral Raises',
                                'Resistance Band Shoulder Press', 'Resistance Band Chest Press',
                                'Resistance Band Seated Leg Press', 'Resistance Band Tricep Extensions',
                                'Resistance Band Seated Ab Twists ', 'Resistance Band Seated Kickbacks', 'Resistance Band Front Raises']
    ...
    def create_widgets(self):
        ...
        self.exercise_label.place(relx=0.85, rely=0.1, anchor=CENTER)
