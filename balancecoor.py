...
class BalanceCoordination():

    def __init__(self, balance):
        self.balance = balance
        self.click_count = 0
        self.balance.after(1000, self.open_balance)
        self.balance.title("FitInclusive")
        self.balance.rowconfigure(0, weight=1)
        self.balance.columnconfigure(0, weight=1)
        height = 720
        width = 1280
        x = (self.balance.winfo_screenwidth() // 2) - (width // 2)
        y = (self.balance.winfo_screenheight() // 2) - (height // 2)
        self.balance.geometry('{}x{}+{}+{}'.format(width, height, x, y))
        self.balance.resizable(0, 0)
        self.balance.config(bg="#291C4F")
        self.myfont = CTkFont(family='Poppins', size=14, weight='bold', slant='italic')

        self.img_path1 = "D:\\pythonProject1\\pict\\Balance exercise.png"
        self.img_path2 = "D:\\pythonProject1\\pict\\bnc.png"#if not self.is_go else "C:\\Users\\Hanseok\\Documents\\project sda\\bgrounded4.png"
        self.img_path3 = "D:\\pythonProject1\\pict\\backbutton2.png"
        self.load_image()
        self.main_frame()

        def exit():
            sure = messagebox.askyesno("Exit", "Are you sure you want to exit", parent=self.balance)
            if sure:
                self.balance.destroy()
        
        self.balance.protocol("WM_DELETE_WINDOW", exit)
    ...
    def main_frame(self):
        ...
        frame_utama = CTkFrame(self.balance, fg_color="#291C4F", bg_color="#291C4F")
    ...
    def tts_balance(self):
        speech = gTTS("balance and coordination exercise", lang="en")
        filepath = ("bnce.mp3")
        speech.save(filepath)
        try:
            playsound.playsound(filepath)
...
class Timer:
    def __init__(self, timer):
        ...
        self.exercise_names = ['One-Leg Balance', 'Toe Taps', 'Heel-to-Toe-Walk', 'Standing March', 'Side Leg Raises',
                               'Single-Leg Deadlift', 'Balance on Wobble Cushion', 'Coordination Ball Toss', 'Arm Reach and Stretch',
                               'Leg Lifts']
...
if __name__ == "__main__":
    balance = Tk()
    app = BalanceCoordination(balance)
    balance.mainloop()
