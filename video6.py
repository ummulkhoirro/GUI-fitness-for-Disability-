...
class AudioPlayerApp:
    def __init__(self, root):
        ...
        self.vid1bg = ImageTk.PhotoImage(Image.open("images/vid6bg.png").resize((root.winfo_screenwidth(),root.winfo_screenheight())))
        ...
        backimg = Image.open("images/back1.png")
        backimg = backimg.resize((120, 80))
        self.backbg = ImageTk.PhotoImage(backimg)
        back = tk.Button(self.root, image=self.backbg, width=100, height=70, fg="white", border=False, background="white",
                      command=openmeditasi)
        back.place(x=180, y=90)
    ...
    def play(self):
        if not self.playing:
            pygame.mixer.init()
            pygame.mixer.music.load("sounds/vid6.mp3")
            pygame.mixer.music.play()

            song_info = pygame.mixer.Sound("sounds/vid6.mp3")
