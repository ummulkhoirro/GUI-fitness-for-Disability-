...
class RythmicDance():
    ...
    def __init__(self, Rythm):
        self.Rythm = Rythm
        self.click_count = 0
        self.Rythm.after(1000, self.open_rythmic)
        self.Rythm.title("FitInclusive")
        self.Rythm.rowconfigure(0, weight=1)
        self.Rythm.columnconfigure(0, weight=1)
        height = 720
        width = 1280
        x = (self.Rythm.winfo_screenwidth() // 2) - (width // 2)
        y = (self.Rythm.winfo_screenheight() // 2) - (height // 2)
        self.Rythm.geometry('{}x{}+{}+{}'.format(width, height, x, y))
        self.Rythm.resizable(0, 0)
        self.Rythm.config(bg="#291C4F")
        self.myfont = CTkFont(family='Poppins', size=14, weight='bold', slant='italic')

        self.img_path1 = "D:\\pythonProject1\\pict\\Rythmics dance.png"
        self.img_path2 = "D:\\pythonProject1\\pict\\rythmicdance.png" #if not self.is_go else "C:\\Users\\Hanseok\\Documents\\project sda\\bgrounded4.png"
        self.img_path3 = "D:\\pythonProject1\\pict\\backbutton2.png"
        self.load_image()
        self.main_frame()

        def exit():
            sure = messagebox.askyesno("Exit", "Are you sure you want to exit", parent=self.Rythm)
            if sure:
                self.Rythm.destroy()
        
        self.Rythm.protocol("WM_DELETE_WINDOW", exit)
    ...
    def main_frame(self):
        ...
        frame_utama = CTkFrame(self.Rythm, fg_color="#291C4F", bg_color="#291C4F")
    ...
    def tts_rythmic(self):
        speech = gTTS("Rythmic Dance", lang="en")
        filepath = ("ecw.mp3")
        speech.save(filepath)
        try:
            playsound.playsound(filepath)
...
class Timer:
    def __init__(self, timer):
        ...
        self.calories_list = [8, 8, 8, 8, 8, 7, 8, 7, 10]
        self.total_time = 0
        self.total_calories = 0

        self.exercise_names = ['Marching in Place', 'Arm Circles', 'Side Streps', 'Knee Lifts', 'Step Touches',
                               'Grapevine Steps', 'Jazz Hands', 'Twist Dance', 'Heel Digs', 'Hip Sways']
