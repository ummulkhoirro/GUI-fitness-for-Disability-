...
class SeatedMarching():
    ...
    def __init__(self, seated):
        self.seated = seated
        self.click_count = 0
        self.seated.after(1000, self.open_seatedmarch)
        self.seated.title("FitInclusive")
        self.seated.rowconfigure(0, weight=1)
        self.seated.columnconfigure(0, weight=1)
        height = 720
        width = 1280
        x = (self.seated.winfo_screenwidth() // 2) - (width // 2)
        y = (self.seated.winfo_screenheight() // 2) - (height // 2)
        self.seated.geometry('{}x{}+{}+{}'.format(width, height, x, y))
        self.seated.resizable(0, 0)
        self.seated.config(bg="#291C4F")
        self.myfont = CTkFont(family='Poppins', size=14, weight='bold', slant='italic')

        self.img_path1 = "D:\\pythonProject1\\pict\\Seated marching.png"
        self.img_path2 = "D:\\pythonProject1\\pict\\seatedmarch.png" 
        self.img_path3 = "D:\\pythonProject1\\pict\\backbutton2.png"
        self.load_image()
        self.main_frame()

        def exit():
            sure = messagebox.askyesno("Exit", "Are you sure you want to exit", parent=self.seated)
            if sure:
                self.seated.destroy()
        
        self.seated.protocol("WM_DELETE_WINDOW", exit)
    ...
    def main_frame(self):
        ...
        frame_utama = CTkFrame(self.seated, fg_color="#291C4F", bg_color="#291C4F")
    ...
    def tts_seatedmarch(self):
        speech = gTTS("seated marching", lang="en")
        filepath = ("stdmrc.mp3")
        speech.save(filepath)
        try:
            playsound.playsound(filepath)
    ...
    def open_seatedmarch(self):
        self.tts_seatedmarch()
...
class Timer:
    def __init__(self, timer):
        ...
        self.calories_list = [6, 5, 6, 8, 6, 4, 5, 7, 6, 7]
        self.total_time = 0
        self.total_calories = 0

        self.exercise_names = ['Seated Marching', 'Seated Arm Raises', 'Seated Leg Lifts', 'Seated Toe Taps', 'Seated Shoulder Shrugs',
                               'Seated Knee Lifts', 'Seated Side Reaches', 'Seated Heel Taps', 'Seated Arm Circles', 'Seated Forward']
