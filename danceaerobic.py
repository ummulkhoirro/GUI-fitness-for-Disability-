...
class DanceAerobic():
    ...
    def __init__(self, daerob):
        self.daerob = daerob
        self.daerob.title("FitInclusive")
        self.daerob.rowconfigure(0, weight=1)
        self.daerob.columnconfigure(0, weight=1)
        height = 720
        width = 1280
        x = (self.daerob.winfo_screenwidth() // 2) - (width // 2)
        y = (self.daerob.winfo_screenheight() // 2) - (height // 2)
        self.daerob.geometry('{}x{}+{}+{}'.format(width, height, x, y))
        self.daerob.resizable(0, 0)
        self.daerob.config(bg="#291C4F")
        self.myfont = CTkFont(family='Poppins', size=14, weight='bold', slant='italic')

        self.img_path1 = "D:\\pythonProject1\\pict\\Dance aerobic.png"
        self.img_path2 = "D:\\pythonProject1\\pict\\danceaerobic.png"
        self.img_path3 = "D:\\pythonProject1\\pict\\backbutton2.png"
        self.load_image()
        self.main_frame()

        def exit():
            sure = messagebox.askyesno("Exit", "Are you sure you want to exit", parent=self.daerob)
            if sure:
                self.daerob.destroy()
        
        self.daerob.protocol("WM_DELETE_WINDOW", exit)
    ...
    def main_frame(self):
        ...
        frame_utama = CTkFrame(self.daerob, fg_color="#291C4F", bg_color="#291C4F")
...
class Timer:
    def __init__(self, timer):
        ...
        self.exercise_names = ['Marching in Place', 'Side Step Touch', 'Grapevine', 'Knee Lifts',
                                'Jumping Jacks', 'High Knees' ,'Butt Kicks' ,'Box Step' ,'Twist',
                                'Cool Down Stretch']
