...
class CardioPole():
    ...
    def __init__(self, pole):
        self.pole = pole
        self.pole.title("FitInclusive")
        self.pole.rowconfigure(0, weight=1)
        self.pole.columnconfigure(0, weight=1)
        height = 720
        width = 1280
        x = (self.pole.winfo_screenwidth() // 2) - (width // 2)
        y = (self.pole.winfo_screenheight() // 2) - (height // 2)
        self.pole.geometry('{}x{}+{}+{}'.format(width, height, x, y))
        self.pole.resizable(0, 0)
        self.pole.config(bg="#291C4F")
        self.myfont = CTkFont(family='Poppins', size=14, weight='bold', slant='italic')

        self.img_path1 = "D:\\pythonProject1\\pict\\Cardio pole.png"
        self.img_path2 = "D:\\pythonProject1\\pict\\cardiopole.png" 
        self.img_path3 = "D:\\pythonProject1\\pict\\backbutton2.png"
        self.load_image()
        self.main_frame()

        def exit():
            sure = messagebox.askyesno("Exit", "Are you sure you want to exit", parent=self.pole)
            if sure:
                self.pole.destroy()
        
        self.pole.protocol("WM_DELETE_WINDOW", exit)
    ...
    def main_frame(self):
        ...
        frame_utama = CTkFrame(self.pole, fg_color="#291C4F", bg_color="#291C4F")
...
class Timer:
    def __init__(self, timer):
        ...
        self.calories_list = [6, 12, 6, 8, 6, 6, 12, 6, 8, 6]
        self.total_time = 0
        self.total_calories = 0

        self.exercise_names = ['Padling Forward', 'Forward and Up', 'Down,Forward,Up', 'Side-to-side Twists',
                               'Rainbow', 'Padling Forward', 'Forward and Up', 'Down,Forward,Up',
                                 'Side-to-side Twists', 'Rainbow']
