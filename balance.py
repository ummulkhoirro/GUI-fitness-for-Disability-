...
class Balance():
    ...
    def __init__(self, bal):
        self.bal = bal
        self.bal.title("FitInclusive")
        self.bal.rowconfigure(0, weight=1)
        self.bal.columnconfigure(0, weight=1)
        height = 720
        width = 1280
        x = (self.bal.winfo_screenwidth() // 2) - (width // 2)
        y = (self.bal.winfo_screenheight() // 2) - (height // 2)
        self.bal.geometry('{}x{}+{}+{}'.format(width, height, x, y))
        self.bal.resizable(0, 0)
        self.bal.config(bg="#291C4F")
        ...
        self.img_path2 = "D:\\pythonProject1\\pict\\balanceexercise.png"#if not self.is_go else "C:\\Users\\Hanseok\\Documents\\project sda\\bgrounded4.png"
        self.img_path3 = "D:\\pythonProject1\\pict\\backbutton2.png"
        self.load_image()
        self.main_frame()

        def exit():
            sure = messagebox.askyesno("Exit", "Are you sure you want to exit", parent=self.bal)
            if sure:
                self.bal.destroy()
        
        self.bal.protocol("WM_DELETE_WINDOW", exit)
    ...
    def main_frame(self):
        ...
        frame_utama = CTkFrame(self.bal, fg_color="#291C4F", bg_color="#291C4F")
...
class Timer:
    def __init__(self, timer):
        ...
        self.calories_list = [10, 10, 10, 10, 10, 10, 10, 10, 10, 10]
        self.total_time = 0
        self.total_calories = 0

        self.exercise_names = ['Single-Leg Stance', 'Tree Pose', 'Heel-to-Toe Walk ' ,'Standing Knee Hugs' ,'Balance Ball Exercise',
                               'Hip Circles' ,'Toe Taps' ,'Side Leg Raises', 'Arm Reach' ,'Cool Down Stretch']
