...
class StrengthWeight():

    def __init__(self, weight):
        self.weight = weight
        self.click_count = 0
        self.weight.after(1000, self.open_weight)
        self.weight.title("FitInclusive")
        self.weight.rowconfigure(0, weight=1)
        self.weight.columnconfigure(0, weight=1)
        height = 720
        width = 1280
        x = (self.weight.winfo_screenwidth() // 2) - (width // 2)
        y = (self.weight.winfo_screenheight() // 2) - (height // 2)
        self.weight.geometry('{}x{}+{}+{}'.format(width, height, x, y))
        self.weight.resizable(0, 0)
        self.weight.config(bg="#291C4F")
        self.myfont = CTkFont(family='Poppins', size=14, weight='bold', slant='italic')

        self.img_path1 = "D:\\pythonProject1\\pict\\Strength traing weight.png"
        self.img_path2 = "D:\\pythonProject1\\pict\\strengthww.png"
        self.img_path3 = "D:\\pythonProject1\\pict\\backbutton2.png"
        self.load_image()
        self.main_frame()

        def exit():
            sure = messagebox.askyesno("Exit", "Are you sure you want to exit", parent=self.weight)
            if sure:
                self.weight.destroy()
        
        self.weight.protocol("WM_DELETE_WINDOW", exit)
    ...
    def main_frame(self):
        ...
        frame_utama = CTkFrame(self.weight, fg_color="#291C4F", bg_color="#291C4F")
    ...
    def tts_weight(self):
        speech = gTTS("Strength Training with Weights", lang="en")
        filepath = ("rcbe.mp3")
        speech.save(filepath)
        try:
            playsound.playsound(filepath)
...
class Timer:
    def __init__(self, timer):
        ...
        self.calories_list = [8, 8, 8, 8, 8, 8, 8, 8, 8, 8]
        self.total_time = 0
        self.total_calories = 0

        self.exercise_names = ['Bicep Curls', 'Tricep Extensions', 'Shoulder Press', 'Lateral Raises', 'Front Raises',
                               'Seated Rows', 'Chest Press', 'Seated Dumbell Flyes', 'Overhead Tricep Extension', 'Seated Dumbell Squats']
